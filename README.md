...
## Canonical layout format (`.rkt`)

`.rkt` ("rekt") is rekolektion's text-based, PDK-aware layout format.
Generators emit `.rkt` directly; the viz tool reads/writes it; GDS
and Magic `.mag` remain as interchange formats at the edges.
...
Properties the format adds over GDS / `.mag`:

- **PDK-qualified layer names** (`sky130:met1`); unknown layers stay
  visible as `unknown:N/D`.
- **First-class ports** with `dir` + `flags`, attached to geometry.
- **Comments preserved through edits** — provenance for human and
  AI-generated reasoning traces.
- **Imports** for multi-file projects.
- **Diff-friendly text** that survives git review.

Python writer (`rekolektion.io.rkt`):

```python
from rekolektion.io import rkt

doc = rkt.Document(
    header_comments=["generated by my_generator 2026-05-13"],
    cells=[
        rkt.Cell(
            name="bitcell",
            elements=[
...
F# parser/writer lives at `tools/viz/src/Rekolektion.Viz.Core/Rkt/`.

**Full reference**: [`docs/io/rkt.md`](docs/io/rkt.md) — schema,
conventions, Python + F# API, anti-patterns. New code generating
layouts should target `.rkt` and read that doc end-to-end first.

## Documentation

- [`docs/io/rkt.md`](docs/io/rkt.md) — Canonical layout format (`.rkt`) reference, schema, conventions, Python + F# APIs
