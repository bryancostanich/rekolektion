...
> **2026-05-03 STATE OF THE WORLD: all 5 original P0s are RESOLVED.**
> See `audit/signoff_gate.md:97-105` for the resolution table and
> evidence trail.  P1 items have written waivers in `docs/waivers/`.
> The detail rows below preserve the audit trail of how each finding
> was originally surfaced; the **Status** column in each row has been
> brought forward.  When in conflict with this notice, `signoff_gate.md`
> is authoritative.
...
> **2026-05-03 update.** Authoritative state lives in
> [`signoff_gate.md`](signoff_gate.md).  All 5 original P0s are
> RESOLVED.  P1 items have written waivers in `docs/waivers/`.
> The historical block below is preserved for audit trail —
> **read it as "what was open at audit start," not as current state.**

### Current (post-resolution) — see signoff_gate.md for evidence

| Severity | Open count | Notes |
|----------|------------|-------|
| P0 | **0** | T1.1-A → tasks #81–#84; T2.1-CIM-A → Phase 2 drain bridge; T5.2-A → Path 3 tap supercell; T4.2-CIM-A → task #79; T4.4-A downgraded P1 + waivered |
| P1 | **0 unresolved** (4 originals all addressed) | T1.4-A reviewed; T1.7-A + T4.1-DIVERGENT-A → `docs/waivers/liberty_timing_analytical.md`; T4.4-A → `docs/waivers/nwell_bias_disclosure.md` |
| P2 | 3 deferable | T1.2-B (dead caches), T4.1-MBL_SENSE-A (docstring drift), T4.1-6T_LR_CIM-A (extract count drift) |

**Sign-off gate (per `trust_audit.md:259-269`):**
- 0 P0 unresolved — currently **0** ✓
- 0 P1 unresolved OR written waiver per item — covered ✓

### Historical (audit-discovery state, pre-resolution)
...
| P0 # | Issue | Pattern | Resolution |
|------|-------|---------|------------|
| 1 | T1.1-A | Production refspice self-extracts (verification-pipeline only) | Hand-written subckts (#81–#84) |
| 2 | T2.1-CIM-A (#7) | CIM bitcell BL/BR drains floating — label strip broke intra-cell label-promotion contact | Phase 2 drain bridge (#49, #51–53) |
| 3 | T5.2-A (#8) | CIM LVS rewrites extracted SPICE to mask 1024 NWELL fragments — label-merge fake | Path 3 tap supercell migration (#92–96) |
| 4 | T4.2-CIM-A (#11) | CIM sense-row VDD floating — no MCON, label-merge fake | Per-column MCONs (#79) |

T4.4-A downgraded P0→P1: investigation of option A' (full sky130 tile pattern) confirmed the floating-NWELL pattern is **structural to sky130 SRAM design** — even with strap between every column + dummy ring, ≥9 floating clusters remain due to bitcell NWELL geometry. Foundry's intended biasing is via subsurface conduction from strap-anchored NWELLs. This is sky130-SRAM-standard practice. Recategorized as risk-disclosure (power-up settle time, latch-up margin, Liberty-not-characterized-at-settle). Waivered via `docs/waivers/nwell_bias_disclosure.md`.

**Audit findings: 4 real silicon defects (P0, all resolved) + 1 design-disclosure (P1, downgraded T4.4-A, waivered) + 3 verification gaps (P1, addressed/waivered).** All P0 silicon defects shared one root cause: net has no metal path AND no parasitic biasing path → broken connectivity, not body bias. Pattern: every per-cell-replicated electrical net needs a flood-fill before any LVS-clean claim. The flood-fill rule is now codified in `signoff_gate.md:84-91` and reverified post-resolution in `audit/flood_fill_2026-05-03.md`.
