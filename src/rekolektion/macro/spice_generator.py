...
from rekolektion.macro.row_decoder import _SPLIT_TABLE
...
def _write_header(f: TextIO, p: MacroParams) -> None:
    f.write(
...
def _top_ports(p: MacroParams) -> list[str]:
    ...
    #
    # Order MUST also match Magic's ext2spice emission order.  Magic
    # walks parent-cell labels in encounter order; for our assembler
    # it emits `addr[0..N-1]` before `clk we cs`.  Listing addr first
    # here matches and avoids netgen's "Top level cell failed pin
    # matching" diagnostic, which fires on positional difference even
    # when the netlist topologically matches.
    ports: list[str] = [f"addr[{i}]" for i in range(p.num_addr_bits)]
    ports += ["clk", "we", "cs"]
...
def _write_wl_driver_row_subckt(f: TextIO, p: MacroParams) -> None:
...
def _write_sense_amp_row_subckt(f: TextIO, p: MacroParams) -> None:
    name = f"sa_{_tag(p)}"
...
def _write_write_driver_row_subckt(f: TextIO, p: MacroParams) -> None:
    name = f"wd_{_tag(p)}"
...
def _write_control_logic_subckt(f: TextIO, p: MacroParams) -> None:
