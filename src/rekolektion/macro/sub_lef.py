...
from rekolektion.macro.row_decoder import _SPLIT_TABLE, _NAND_DEC_PITCH
...
def lef_ctrl_logic(p: MacroParams, ctrl_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
...
def lef_row_decoder(p: MacroParams, rd_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
...
def lef_wl_driver(p: MacroParams, wld_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
...
def lef_sense_amp_row(p: MacroParams, sa_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
...
def lef_write_driver_row(p: MacroParams, wd_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
    name = f"wd_{_tag(p)}"
    pins: list[LefPin] = []
    pitch = p.mux_ratio * _BITCELL_WIDTH
    for i in range(p.bits):
...
def lef_precharge_row(p: MacroParams, pre_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
...
def lef_column_mux_row(p: MacroParams, mux_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
...
def lef_bitcell_array(p: MacroParams, arr_size: tuple[float, float]) -> tuple[str, list[LefPin]]:
