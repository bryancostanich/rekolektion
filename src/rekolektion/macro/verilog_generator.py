...
def _row_decoder_ports(p: MacroParams) -> list[tuple[str, str]]:
...
def _wl_driver_ports(p: MacroParams) -> list[tuple[str, str]]:
    ports: list[tuple[str, str]] = []
    for r in range(p.rows):
...
def _sa_ports(p: MacroParams) -> list[tuple[str, str]]:
    ports: list[tuple[str, str]] = []
...
def _wd_ports(p: MacroParams) -> list[tuple[str, str]]:
    ports: list[tuple[str, str]] = []
    for i in range(p.bits):
...
def _column_mux_ports(p: MacroParams) -> list[tuple[str, str]]:
    return (
...
def _write_blackboxes(f: TextIO, p: MacroParams) -> None:
    tag = _tag(p)
...
def _write_top_module(f: TextIO, p: MacroParams) -> None:
    tag = _tag(p)
