...
module Rekolektion.Viz.Core.Gds.Reader
...
/// Parse XY record data into a list of points (raw DBU, int64).
let private parseXY (data: byte[]) : Point list =
...
let readGds (path: string) : Library =
    ...
    let mutable libName = ""
    let mutable userUnitsPerDb = 0.001  // default: 1 user unit = 1 um, 1 db unit = 1 nm
    let mutable dbUnitsMeters = 1e-9    // default: 1 db unit = 1 nm

    let structures = System.Collections.Generic.List<Structure>()
    let mutable currentStructName = ""
    let mutable currentElements = System.Collections.Generic.List<Element>()
    ...
    let mutable elemPoints : Point list = []
    let mutable elemSName = ""
    let mutable elemString = ""
    ...
    for record in records do
        ...
        if rt = RecordType.LIBNAME then
            ...
            // FIRST real8: user units per database unit (e.g. 0.001 μm/DBU)
            // SECOND real8: meters per database unit (e.g. 1e-9 for nm)
            userUnitsPerDb <- readReal8 d 0
            dbUnitsMeters <- readReal8 d 8

        elif rt = RecordType.BGNSTR then
            currentStructName <- ""
            currentElements <- System.Collections.Generic.List<Element>()
        ...
        elif rt = RecordType.STRING then
            elemString <- readString d
            ...
            | "SREF" ->
                let origin = if elemPoints.Length > 0 then elemPoints.[0] else { X = 0L; Y = 0L }
            ...
            | "AREF" ->
                let origin = if elemPoints.Length > 0 then elemPoints.[0] else { X = 0L; Y = 0L }
                let colVec = if elemPoints.Length > 1 then elemPoints.[1] else { X = 0L; Y = 0L }
                let rowVec = if elemPoints.Length > 2 then elemPoints.[2] else { X = 0L; Y = 0L }
                let reflected = elemStrans &&& 0x8000us <> 0us
    ...
    {
        Name = libName
        UserUnitsPerDbUnit = userUnitsPerDb
