...
// --- Check.runLive Compat.Magic true end-to-end ------------------------------------------
...
let ``runLive on draft violating met1 spacing fires the violation`` () =
    ...
    let v = Check.runLive Compat.Magic true Rules.defaultView units1nm cell draftFlat Map.empty None Set.empty
...
let ``runLive with empty cell and empty draft → no violations`` () =
    Check.runLive Compat.Magic true Rules.defaultView units1nm [||] [||] Map.empty None Set.empty
    |> should be Empty
...
let ``runLive with empty draft → no violations even on dirty cell`` () =
    ...
    |]
    Check.runLive Compat.Magic true Rules.defaultView units1nm cell [||] Map.empty None Set.empty
    |> should be Empty
...
let ``runLive region-filter skips cell polys far outside the draft window`` () =
    ...
    let draftFlat = Draft.toFlatPolygons draftSegs
    Check.runLive Compat.Magic true Rules.defaultView units1nm cell draftFlat Map.empty None Set.empty
    |> should be Empty
...
let ``runLive perf bound: 1000 cell polys + 2-segment draft under 100 ms`` () =
    ...
    let _ = Check.runLive Compat.Magic true Rules.defaultView units1nm cell draftFlat Map.empty None Set.empty
...
let ``runLive with a custom view fires only the view's rules`` () =
    ...
    let draftFlat = Draft.toFlatPolygons draftSegs
    let v = Check.runLive Compat.Magic true tightView units1nm cell draftFlat Map.empty None Set.empty
...
let ``runLive with an empty view fires no rule violations on dirty geometry`` () =
    ...
    let draftFlat = Draft.toFlatPolygons draftSegs
    let v = Check.runLive Compat.Magic true emptyView units1nm cell draftFlat Map.empty None Set.empty
...
let ``runLive honors disabledRules from the caller`` () =
    ...
    let v = Check.runLive Compat.Magic true Rules.defaultView units1nm cell draftFlat Map.empty None disabled
    v |> Array.exists (fun x -> x.Rule = "met1.2")
      |> should equal false
